...
public class EsamiResource {
    ...
    @GetMapping("/esamis")
    @Timed
    public List<EsamiDTO> getAllEsamis() {
        log.debug("REST request to get all Esamis");
        return esamiService.findAll();
        }
    ...
    public ResponseEntity<EsamiDTO> getEsami(@PathVariable Long id) {
        ...
        EsamiDTO esamiDTO = esamiService.findOne(id);
        return ResponseUtil.wrapOrNotFound(Optional.ofNullable(esamiDTO));
    }
